...
This repository witnesses the Solace decision chain head every day.
...
record before it. This repository publishes the chain head daily so anyone can
verify ordering and integrity without trusting Solace infrastructure.
...
👉 [Verify today's anchor in your browser](https://solace.fyi/anchor)

## Daily publication

A GitHub Actions workflow in `.github/workflows/daily-anchor.yml` runs every day
at 12:34 UTC. It fetches the live Hermes decision ledger, writes
`YYYY-MM-DD.json`, verifies continuity, and commits the result.

To generate today's anchor manually:
...
- The chain head on date X was hash Y.
- Each anchor's `previous_anchor` matches the previous day's `chain_head`.
...
```json
{
  ...
  "source_url": "https://solace.fyi/anchor/2026-08-09"
}
```
